...
- `histogram`: which is implemented in the orginal project ["make-activity"](https://github.com/STAT545-UBC/make-activity). It shows a histogram of frequencies of letters in a document called "words.txt". You can run `make histogram` to run this target alone.
- `sort_country`: it obtains countries in `gapminder` dataframe that also appears in "words.txt", and shows maximum population in different formats. You can run `make sort_country` to run this target alone.
